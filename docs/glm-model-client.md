...
`GlmModelClient` 提供流式对话、任务规划、Token 计数、上下文压缩和连接关闭。所有文本生成都复用 `create_response_stream()`，不再提供非流式 `create_response()`。
...
async def main():
    ...
    try:
        response = await client.create_response_stream(
            ModelRequest(
                model="GLM-5.3-flash",
...
- `AgentLoop.run()`：通过 `on_text_delta(text)` 通知正文，通过 `on_reasoning_delta(text)` 通知 GLM 的思考片段。`on_message_end()` 仅在有完整正文时触发；`on_response_end()` 在每次完整模型响应后触发，包含只有思考和工具调用的响应。聊天终端展示临时思考区，正文开始后清除思考区，流式预览正文，结束后保留 Markdown 结果；工具调用信息不展示。taskplan 的规划、步骤执行和上下文压缩也使用流式请求；这些内部流程收齐内容后再解析返回，不向聊天正文输出中间片段。见 [终端界面](terminal-ui.md)。
...
- `create_task_plan`：复用流式接口并开启 JSON 模式，提供 TaskPlan schema 和工具描述，再通过 Pydantic 校验返回结果。规划请求不注册可执行工具；无效计划会抛出校验异常。
...
- `compact_context`：复用流式接口生成摘要，返回带有“历史对话摘要”前缀的 system 消息。
